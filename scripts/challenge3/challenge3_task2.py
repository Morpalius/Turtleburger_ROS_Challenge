...
def line_follower(img):
    ...
    #le Stop_flag "dans_tunnel" permet de verifier si le robot n'est pas dans le tunnel. Car si il est dedans --> pas de ligne jaune , 
    #mais on veut que le robot marche pour autant
...
def open_door(img):
    ...
    global door_flag #indique le passage de la porte si door_flag = 0
    ...
    if len(blue_pts)>=5 and Stop_flag == 1:
        ...
        var = True
        pub.publish(var)
        #passage du door flag à zero une fois la porte passée, informe de l'entrée dans le tunnel
        if door_flag == 1 : print("Ouverture Porte")
        door_flag = 0


def wall_follower(msg):  


#Fonction du subscriber "sub_wall" utilisant les informations du lidar pour un suivi de murs dans un tunnel
  global Stop_flag #voir initialisation du flag dans le main
  ...
  if dans_tunnel ==1:
      Stop_flag=0 #flag à 0 pour inhiber l'action depuis la fonction de suivi de ligne 
...
if __name__ =='__main__':
  ...
  Stop_flag=0 #flag qui est = 1 si le robot doit bouger et =0 si il doit s'arreter
  door_flag = 1 #flag de controle de passage de la porte bleue 
  
  rospy.init_node('line_follower_plus_wall')
